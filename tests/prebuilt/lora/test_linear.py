...
def test_lora_linear(config, base_layer_bias, dropout):
    ...
    with torch.no_grad():
        ...
        assert torch.allclose(activated_adapter_output, adapted_layer(sample))

        assert torch.allclose(
            activated_adapter_output, adapted_layer.base_layer(sample)
        ...
        assert not torch.allclose(activated_adapter_output, adapted_layer(sample))

        # Everything like the original output
        assert torch.allclose(
            output, o := adapted_layer(sample), atol=1e-5
        ), f"abs = {(o-output).abs().max()}"  # ~9e-6
